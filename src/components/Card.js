...
function Card({ name, link, id, likes, owner, onCardClick, onCardLike, onCardDelete }) {
  ...
  const cardLikeButtonClassName = ( 
    `photo-item__like-btn ${isLiked && 'photo-item__like-btn_active'}` 
  );
  ...
  return (
    <li className="photo-item">
      {isOwn && <button  className="photo-item__delete-btn" type="button" onClick={handleDeleteClick} />}
      {/* <button className="photo-item__delete-btn" type="button" /> */}
      <img className="photo-item__img" src={link} alt={name} onClick={handleClick} />
